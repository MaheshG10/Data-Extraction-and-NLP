...
with open(filename, 'w', newline='') as csvfile:
  ...
  for index, row in input_df.iterrows():
    try:
      url_id = row['URL_ID']
      # Define the URL of the webpage to be analyzed
      ...
      # words = re.findall('\w+', article_tag.get_text().strip().split())
      # with open('37.0.txt', 'r') as file:
      #     text = file.read()
      text = article_text
      words=text.split()
